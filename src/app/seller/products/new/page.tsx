...
import { ArrowLeft, Plus, Trash2, ArrowRight, Package, Clock, ShieldCheck, Info, CheckCircle2, Sparkles } from "lucide-react";
...
export default function AddProductPage() {
  ...
  const isRequestMode = category === REQUEST_NEW_VALUE;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRequestMode) {
        // Create category approval request for admin
        ...
        // Do not redirect immediately — show success state with regulatory timeline
      } else {
      ...
      alert(isRequestMode ? "Failed to submit category request. Please try again." : "Failed to publish material pack. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-white border border-emerald-200 rounded-2xl p-8 shadow-sm text-center space-y-5">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-left space-y-3">
            ...
            <div className="flex items-start gap-2.5 pt-2 border-t border-amber-200/60">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                You’ll receive an email at <span className="font-semibold">{user?.email}</span> upon approval. Your material pack draft is saved and will auto-publish under the new dock once approved. For urgent cases, contact <span className="font-mono text-sky-700">support@bondor.io</span>.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            ...
            <Button variant="default" size="md" onClick={() => { setRequestSuccess(false); setCategory("tshirt"); setProposedName(""); setProposedDesc(""); }}>
              Submit Another
            </Button>
          </div>
        </div>
      ...
      <div className="bg-white border border-zinc-200/90 rounded-xl p-6 sm:p-7 shadow-xs">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {isRequestMode && (
            <div className="space-y-4 bg-amber-50/60 border border-amber-200 rounded-xl p-4 animate-in fade-in">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  rows={3}
                  placeholder="Describe what makers will produce in this dock, typical raw materials, and why it deserves a dedicated harbor dock…"
                  className="w-full px-3.5 py-2 bg-white border border-amber-200 rounded-lg text-sm placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-amber-500"
                  value={proposedDesc}
          ...
          <div className="pt-3 border-t border-zinc-100 flex items-center justify-end gap-3">
            ...
            >
              {isRequestMode ? "Submit for Admin Approval" : "Publish Material Pack"}
            </Button>
          </div>
